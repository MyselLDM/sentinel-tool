...
.DESCRIPTION
  ...
  access for an NDA envelope). prompt.py drops those rather than writing
  mislabelled rows, so the script repeats passes until one records nothing new
  or -MaxPasses runs out.
...
print(count * len(prompt.POLICIES))
"@
