...
def to_probabilities(scores):
    """Return a proper probability distribution over the NLI labels.

    ``CrossEncoder.predict`` may hand back raw logits or already-softmaxed scores
    depending on the sentence-transformers version / ``activation_fn``. Normalize
    so index 0 (contradiction) is a real probability; ``argmax`` is unaffected
    because softmax is monotonic.
    """
    s = np.asarray(scores, dtype=np.float64)
    if np.all(s >= 0.0) and np.all(s <= 1.0) and abs(float(s.sum()) - 1.0) < 1e-3:
        return s  # already a probability distribution
    e = np.exp(s - np.max(s))
    return e / e.sum()


def find_best_threshold(y_true_malicious, p_contradiction, steps=201):
    """Pick the ``p(contradiction)`` cut-off that maximizes malicious-class F1.

    Mirrors ``train_contrastive.evaluate_contrastive`` so the NLI operating point
    is selected the same way -- a single scalar that can be written to
    ``model_config.json`` and reused verbatim at inference.

    Returns ``(threshold, f1_percent)``.
    """
    y_true = np.asarray(y_true_malicious, dtype=int)
    p = np.asarray(p_contradiction, dtype=np.float64)
    best_f1, best_threshold = 0.0, 0.5
    for thresh in np.linspace(0.0, 1.0, steps):
        pred = (p > thresh).astype(int)          # high p(contradiction) => malicious
...
def evaluate_model(model, test_data):
    ...
    Also derives the F1-optimal ``p(contradiction)`` threshold -- the decision
    rule used at inference (see find_best_threshold).
    """
    correct = 0
    total = len(test_data)
    tp, fp, tn, fn = 0, 0, 0, 0
    ...
    for premise, hypothesis, true_label in test_data:
        scores = to_probabilities(model.predict([(premise, hypothesis)])[0])
        ...
        y_true_malicious.append(1 if true_malicious else 0)
        p_contra.append(float(scores[0]))
        details.append({
    ...
    # F1-optimal operating point on p(contradiction) -- the production decision rule
    threshold, threshold_f1 = find_best_threshold(y_true_malicious, p_contra)
...
def finetune_nli():
    ...
    std_thresh = np.std([r["threshold"] for r in fold_results])
    ...
    print(f"  Fine-tuned threshold:         {avg_thresh:.3f} ± {std_thresh:.3f}")
    ...
    # --- Persist CV results + the recommended inference threshold ---
    # Mirrors train_contrastive.py's logs/contrastive_cv_results.json so the two
    # model artifacts can be merged into fastapi/model_config.json (see
    # fastapi/plan.md §3). recommended_nli_threshold = mean of the per-fold
    # F1-optimal p(contradiction) cut-offs (an unbiased, held-out estimate).
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True)
    metrics = ["accuracy", "tpr", "fpr", "precision", "f1",
               "threshold", "threshold_f1"]
    nli_output = {
        "model": "sentinelagent-nli-3class",
        "base": "cross-encoder/nli-MiniLM2-L6-H768",
        "labels": ["contradiction", "entailment", "neutral"],
        "decision": "p_contradiction > threshold",
        "recommended_nli_threshold": float(avg_thresh),
        "folds": [
            {k: (float(v) if isinstance(v, (np.floating, np.integer))
                 else v) for k, v in r.items() if k != "details"}
            for r in fold_results
        ],
        "aggregate": {
            m: {"mean": float(np.mean([r[m] for r in fold_results])),
                "std": float(np.std([r[m] for r in fold_results]))}
            for m in metrics
        },
    }
    result_path = os.path.join(log_dir, "nli_cv_results.json")
    with open(result_path, "w") as f:
        json.dump(nli_output, f, indent=2)
    print(f"\n  Recommended inference threshold (mean across folds): "
          f"{avg_thresh:.3f} ± {std_thresh:.3f}")
    print(f"  Per-fold results saved to {result_path}")
